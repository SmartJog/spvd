...
class BaseJob(object):
    ...
    def __init__(self, options, infos, params):
        ...
        logger_per_job = logging.getLogger(
            "spvd.jobs.%s.%s"
            % (self.infos["check"]["plugin"], self.infos["check"]["plugin_check"])
        )
    ...
    def run(self):
        ...
        if (
            self.infos["check"]["check_infos"].get("history", False) == "true"
            and self.old_status != self.infos["status"]["check_status"]
        ):
            self.log.debug(
                "Saving new history checkpoint: "
                + str(self.old_status)
                + " -> "
    ...
    def go(self):
        ...
        if hasattr(self, self.infos["check"]["plugin_check"]):
            ...
            message = (
                "Job does not implement <%s> method"
                % self.infos["check"]["plugin_check"]
            )
            raise BaseJob.BaseError(message)
